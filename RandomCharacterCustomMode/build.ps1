...
$keep  = @('RandomCharacterCustomMode.dll', 'RandomCharacterCustomMode.pdb', 'RandomCharacterCustomMode.deps.json')

dotnet build "$root\RandomCharacterCustomMode.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\RandomCharacterCustomMode.pck"
...
Write-Host "  pck: $root\RandomCharacterCustomMode.pck"
Write-Host "  dll: $binDebug\RandomCharacterCustomMode.dll"
