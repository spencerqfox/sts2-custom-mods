...
$keep  = @('DoorRemaker.dll', 'DoorRemaker.pdb', 'DoorRemaker.deps.json')
$env:DOTNET_SKIP_FIRST_TIME_EXPERIENCE = '1'
$env:NUGET_PACKAGES = Join-Path $env:USERPROFILE '.nuget\packages'

dotnet build "$root\DoorRemaker.csproj" -c Debug --ignore-failed-sources `
    -p:ModsPath= -p:RestoreSources="$env:NUGET_PACKAGES"
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\DoorRemaker.pck"
...
Write-Host "  pck: $root\DoorRemaker.pck"
Write-Host "  dll: $binDebug\DoorRemaker.dll"
