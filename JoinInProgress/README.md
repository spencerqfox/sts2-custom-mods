...
- Combat rooms present recorded card and relic choices; rest sites offer Rest or Smith; shops use the recorded stock with base prices; chests present recorded relics; events show the recorded event and let the joiner follow or pass on compatible logged changes.
...
Relics with `HasUponPickupEffect` are shown as unavailable during catch-up. Those relics can open nested reward/card-selection screens whose multiplayer synchronization IDs cannot safely be replayed from run history; the joiner can choose another recorded relic or skip that reward.
