...
$keep  = @("$modId.dll", "$modId.pdb", "$modId.deps.json")

dotnet build "$root\$modId.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\$modId.pck"
...
Write-Host "  pck: $root\$modId.pck"
Write-Host "  dll: $binDebug\$modId.dll"
