...
$keep  = @('CorrelatedRandomnessFix.dll', 'CorrelatedRandomnessFix.pdb', 'CorrelatedRandomnessFix.deps.json')

dotnet build "$root\CorrelatedRandomnessFix.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\CorrelatedRandomnessFix.pck"
...
Write-Host "  pck: $root\CorrelatedRandomnessFix.pck"
Write-Host "  dll: $binDebug\CorrelatedRandomnessFix.dll"
