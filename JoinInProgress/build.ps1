...
$keep = @('JoinInProgress.dll', 'JoinInProgress.pdb', 'JoinInProgress.deps.json')

dotnet build "$root\JoinInProgress.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\JoinInProgress.pck"
...
Write-Host "  pck: $root\JoinInProgress.pck"
Write-Host "  dll: $binDebug\JoinInProgress.dll"
