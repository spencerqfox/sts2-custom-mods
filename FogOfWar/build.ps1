...
$keep  = @('FogOfWar.dll', 'FogOfWar.pdb', 'FogOfWar.deps.json')

dotnet build "$root\FogOfWar.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\FogOfWar.pck"
...
Write-Host "  pck: $root\FogOfWar.pck"
Write-Host "  dll: $binDebug\FogOfWar.dll"
