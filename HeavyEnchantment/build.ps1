...
$keep  = @('HeavyEnchantment.dll', 'HeavyEnchantment.pdb', 'HeavyEnchantment.deps.json')

dotnet build "$root\HeavyEnchantment.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\HeavyEnchantment.pck"
...
Write-Host "  pck: $root\HeavyEnchantment.pck"
Write-Host "  dll: $binDebug\HeavyEnchantment.dll"
