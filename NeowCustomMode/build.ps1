...
$keep  = @('NeowCustomMode.dll', 'NeowCustomMode.pdb', 'NeowCustomMode.deps.json')

dotnet build "$root\NeowCustomMode.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\NeowCustomMode.pck"
...
Write-Host "  pck: $root\NeowCustomMode.pck"
Write-Host "  dll: $binDebug\NeowCustomMode.dll"
