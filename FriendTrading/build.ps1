...
$keep  = @('FriendTrading.dll', 'FriendTrading.pdb', 'FriendTrading.deps.json')

dotnet build "$root\FriendTrading.csproj" -c Debug --ignore-failed-sources `
    --source "$env:USERPROFILE\.nuget\packages" -p:ModsPath=
if ($LASTEXITCODE -ne 0) { throw "dotnet build failed ($LASTEXITCODE)" }

& $godot --headless --path $root --export-pack BasicExport "$root\FriendTrading.pck"
...
Write-Host "  pck: $root\FriendTrading.pck"
Write-Host "  dll: $binDebug\FriendTrading.dll"
